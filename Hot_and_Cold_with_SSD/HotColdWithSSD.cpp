...
#define VIDEO                   2000        // 비디오 수
#define END_SIMULATION          86400       // 시뮬레이션 시간, 24h
#define ARRIVAL_RATE            1.0         // 1초당 리퀘스트 수
#define VIDEO_THETA             0.271       // zipf theta
#define SEG_THETA               0.2         // zipf theta
#define LENGTH                  2           // L
...
int                                 totalVideoSize;
double                              videoSize[VIDEO];               // 비디오별 파일 크기
...
vector<bool>                        caching[VIDEO];                 // SSD에 캐싱될 버전
...
double                              ErsRequestSsdServiceTime;
...
void GetMaximumDiskNumber(){
    ...
    maximumDiskNumber = maximumCapacity / (diskCapacity * 0.99);
    if(maximumCapacity % (int)(diskCapacity * 0.99)) maximumDiskNumber++;
}
...
double GetSsdServiceTime(int video, int seg, int ver){
    return (((double)GetChunkCapacity(video, seg, ver) / SSD_TRANSFER_TIME));
}
...
void SetNumberOfDisk(int numberOfDisk){
    ...
    S_limit = (diskCapacity * NUMBER_OF_DISK) * 0.99;
...
void Heuristic_find_X(){
    ...
    if(current_capacity > S_limit){
        ...
        while(current_capacity > (numberOfDisk * diskCapacity) * 0.99) numberOfDisk++;
        SetNumberOfDisk(numberOfDisk);
    }
    int vec_idx = 0;
...
void chunk_allocation(){
    ...
    for(int v = 0; v < VIDEO; v++) caching[v].assign(videoSegmentSize[v], 0);   // ssd caching
    ...
    double curCachingCap = 0;                                                   // ssd caching
    int valSegIdx = valuable_segment.size() - 1;                                // ssd caching
    while(ssdCapacity > curCachingCap){                                             // ssd caching
        double cap = valuable_segment[valSegIdx].second.first;                  // ssd caching
        int video  = valuable_segment[valSegIdx].second.second.first;           // ssd caching
        int seg    = valuable_segment[valSegIdx].second.second.second;          // ssd caching
        caching[video][seg] = 1;

        valSegIdx--;                                                            // ssd caching
        curCachingCap += cap;                                                   // ssd caching
    }


    vector<pair<double, pair<double, int> > > disk;                             // value, capacity, disk_number
    for(int d = 0; d < NUMBER_OF_DISK; d++){
        disk.push_back({0, {diskCapacity, d}});                                         // 디스크 초기화
        remainDiskCapacity[d] = diskCapacity;
    }
...
void bandwidth_allocation(int sec){
    ...
    vector<pair<int, pair<int, pair<int, int> > > > ssdService; // video, seg, ver, selected_version_index
    vector<pair<double, pair<int, pair<int, pair<int, int> > > > > ssdValuableService; // value, service number, video, seg, selected version

    vector<bool> isfinished;
    vector<bool> ssdIsfinished;
    vector<double> usedBandwidth;
    usedBandwidth.assign(NUMBER_OF_DISK, 0);
    double ssdUsedBandwidth = 0;
    ...
    while(pos != tail){
        ...
        for(auto v : hotStoredVersion[video][seg]){
            if(ver <= v){
                if(!initVersionFlag){
                    ...
                    if(caching[video][seg]){
                        double heuristicValue   = (ssim[v][videoSegmentIdx[video][seg]] - ssim[initVersion][videoSegmentIdx[video][seg]]) / (GetSsdServiceTime(video, seg, v) - GetSsdServiceTime(video, seg, initVersion));
                        ssdValuableService.push_back({heuristicValue, {ssdServiceNum, {video, {seg, v}}}});
                    }
                    else{
        ...
        if(caching[video][seg]){
            ssdService.push_back({video, {seg, {ver, initVersion}}});
            ssdServiceNum++;
            ssdUsedBandwidth += GetSsdServiceTime(video, seg, initVersion);
        }
        else{
    ...
    ssdIsfinished.assign(ssdService.size(), 0);
    sort(valuableService.begin(), valuableService.end());
    sort(ssdValuableService.begin(), ssdValuableService.end());
    int finishedCount = 0;
    int ssdFinishedCount = 0;
    ...
    // ssd part
    for(auto valServ : ssdValuableService){
        if(ssdFinishedCount == ssdService.size()) break;
        serviceNum          = valServ.second.first;
        int video           = valServ.second.second.first;
        int seg             = valServ.second.second.second.first;
        int selectedVersion = valServ.second.second.second.second;
        ...
        if(ssdIsfinished[serviceNum]) continue;

        if((ssdUsedBandwidth <= LENGTH)){
            ssdIsfinished[serviceNum] = true;
            ssdFinishedCount++;
            continue;
        }
        if(GetSsdServiceTime(video, seg, selectedVersion) < GetSsdServiceTime(video, seg, prevVersion)){
            ssdService[serviceNum].second.second.second = selectedVersion;
            ssdUsedBandwidth += GetSsdServiceTime(video, seg, selectedVersion) - GetSsdServiceTime(video, seg, prevVersion);
            // ssdChangeCount++;
        }
    }
    ...
    if(ssdUsedBandwidth > LENGTH) printf("| ERROR!! SSD over bandwidth\t\t |\n");

    ErsRequestServiceTime       = 0;
    OriginalRequestServiceTime  = 0;

    ErsRequestSsdServiceTime    = 0;
    // hdd part
    for(auto sv : service){
        int video               = sv.first;
    ...
    // ssd part
    for(auto sv : ssdService){
        int video               = sv.first;
        int seg                 = sv.second.first;
        int ver                 = sv.second.second.first;
        int selectedVersion     = sv.second.second.second;

        ErsQoE      += ssim[selectedVersion][videoSegmentIdx[video][seg]];
        OriginalQoE += ssim[ver][videoSegmentIdx[video][seg]];

        ErsPower        += ((GetChunkCapacity(video, seg, selectedVersion) / SSD_TRANSFER_TIME) * SSD_ACTIVE_POWER);
        OriginalPower   += ((GetChunkCapacity(video, seg, ver) / TRANSFER_TIME) * ACTIVE_POWER);

        ErsRequestSsdServiceTime    += (GetChunkCapacity(video, seg, selectedVersion) / SSD_TRANSFER_TIME);
        OriginalRequestServiceTime  += (GetChunkCapacity(video, seg, ver) / TRANSFER_TIME);
    }
    serviceCount += service.size() + ssdService.size();

    hEnd = clock();
    if(sec && sec%3600 == 0) cout<<sec<<"s BAA execution time: "<<(hEnd-hBegin)<<"ms"<<endl;
}
...
int ssdSize[3] = {0, 524288, 1048576};
int diskSize[2] = {2097152, 3145728};

string GetInputFileName(){
    return to_string(END_SIMULATION) + "_" + to_string(ARRIVAL_RATE) + "_" + "Request_Input.txt";
}

string GetOutputFileName(int settingDiskNum, int ssdCapIdx, int diskSizeIdx){
    string ret = !workload ? "./HVP_" : "./LVP_";
    ret += !ssdCapIdx ? "" : ssdCapIdx == 1 ? "SSD500GB_" : "SSD1TB_";
    ret += !diskSizeIdx ? "DISK2TB/" : "DISK3TB/";
    return ret + "GB_Video" + to_string(VIDEO) + "_Duration" + to_string(END_SIMULATION) + "_ArrivalRate" + to_string(ARRIVAL_RATE) + "_SetDisk" + to_string(settingDiskNum) + "_MaxDisk" + to_string(maximumDiskNumber) + "_Workload(" + to_string(workload) + ").txt";
}

int workLoadIdx[2] = {_HVP, _LVP};

int main(int argc, char* argv[]){
    int settingDiskNum = atoi(argv[1]);
    workload = workLoadIdx[atoi(argv[2])];
    int ssdCapIdx      = atoi(argv[3]);
    ssdCapacity        = ssdSize[ssdCapIdx];
    int diskSizeIdx    = atoi(argv[4]);
    diskCapacity       = diskSize[diskSizeIdx];
    clock_t begin, end;
    begin = clock();
...
//    freopen("ttteeesssttt.txt", "w+", stdout);
    ...
    for(int video = 0; video < VIDEO; video++){
        ...
        int startIdx, endIdx;
        endIdx = VideoNum(video);
        if(endIdx == 102) startIdx = 0;
        else if(endIdx == 224) startIdx = 103;
        else if(endIdx == 362) startIdx = 225;
        else if(endIdx == 512) startIdx = 363;
        else startIdx = 513;
        uniform_int_distribution<unsigned int> segment_generator(startIdx, endIdx);
        // uniform_int_distribution<unsigned int> segment_generator(0, endIdx);
...
//    PrintPopularity();
    ...
    freopen(GetOutputFileName(settingDiskNum, ssdCapIdx, diskSizeIdx).c_str(), "w+", stdout);


    // minimumDiskServiceTime();
    SetNumberOfDisk(settingDiskNum);
    Heuristic_find_X();
    // printX();
    ...
    for(int sec = 0, client = 0; sec <= END_SIMULATION; sec++){
        ...
        int ersSsdIdleTime   = (LENGTH - ErsRequestSsdServiceTime);

        if(ersIdleTime < 0) printf("ers IDLE ERROR sec: %d\n", sec);
        if(originalIdleTime < 0) printf("ori IDLE ERROR sec: %d\n", sec);
        if(ersSsdIdleTime < 0) printf("ers SSD IDLE ERROR sec %d\n", sec);

        ErsPower        += (ersIdleTime * IDLE_POWER) + ersSsdIdleTime * SSD_SLUMBER_POWER;
...
//    printf("\n%d\n", changeCount);
    ...
    cout<<"\n\n\nMaximum Disk: "<<maximumDiskNumber<<"\nexecution time : "<<(end-begin)<<"ms"<<endl;
