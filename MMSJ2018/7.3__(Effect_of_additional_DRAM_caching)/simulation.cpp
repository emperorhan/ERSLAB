...
string GetOutputFileName(int nlimit, int windowsize, int opt, int ssdBand, int dramSize){
	string ret = opt == 0 ? "SPC_throttling_" : "SPC_";
    return ret + "disk_bandwidth_saved___(N_limit=" + to_string(nlimit) + ",windowSize=" + to_string(windowsize) + ",ssdBand=" + to_string(ssdBand) + ",dramSize=" + to_string(dramSize) + ").txt";
}

int ssdBandArr[4] = {_1GB, _2GB, _3GB, _4GB};
int dramSizeArr[3] = {_0GB, _16GB, _32GB};

int main(int argc, char* argv[]) {
	int n_limit = 1000;
	int groupsize = 56;
	int opt = 1;
	int ssdBandIdx = atoi(argv[1]);
	int dramSizeIdx = atoi(argv[2]);
	
	MAX_SSD	 = memSize[ssdBandArr[ssdBandIdx]]; 	//0:1GB 1:2GB 2:3GB 3:4GB
	MAX_DRAM = memSize[dramSizeArr[dramSizeIdx]];	//0:0GB 1:16GB 2:32GB

	freopen(GetInputFileName(n_limit, groupsize, opt).c_str(), "r", stdin);
	freopen(GetOutputFileName(n_limit, groupsize, opt, MAX_SSD, MAX_DRAM).c_str(), "w+", stdout);
	...
	for (int period = 1; period <= CHANGE; period++)
	{
		...
		for (int i = 2; i <= SIMULATION_TERM; i++)
		{
			...
			if (i >= HERE)
				sum += only_disk_use[i];
			if ((i >= HERE) && (i <= (SIMULATION_TERM - HERE)))
			{
