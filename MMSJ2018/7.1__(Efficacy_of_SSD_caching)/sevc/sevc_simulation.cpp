//파일 실행 시 SSD Size, SSD Bandwidth를 옵션으로 ex: ./execution 0 0 ~ ./execution 3 1
#include <cstdio>
#include <algorithm>
#include <vector>
#include <queue>
...
string GetInputFileName(int ssdSize){
	return "SEVC_poisson___(ssdSize=" + to_string(ssdSize) + ").txt";
}

string GetOutputFileName(int ssdSize, int ssdBandwidthIdx){
	return "SEVC_Disk_bandwidth_saved___(ssdSize=" + to_string(ssdSize) + "_ssdBandwidth=" + to_string(memSize[maxSSDidx[ssdBandwidthIdx]]) + "KB).txt";
}

int main(int argc, char* argv[]) {
	ssdSizeIdx = atoi(argv[1]);			//32MB기준 0.5=16384 1TB=32768 1.5=49152 2TB=65536
	int ssdBandwidthIdx = atoi(argv[2]);
	freopen(GetInputFileName(ssdSizeArr[ssdSizeIdx]).c_str(), "r", stdin);
	freopen(GetOutputFileName(ssdSizeArr[ssdSizeIdx], ssdBandwidthIdx).c_str(), "w+", stdout);
	
	MAX_DRAM = (double)memSize[_0GB];
	...
	for (int period = 1; period <= CHANGE; period++)
	{
		...
		for (int i = 1; i <= NUMBER_OF_VIDEO; i++)
		{
			...
			if(prefix[i]){
				prefix[i] *= _U;
				if(prefix[i]) prefix[i] /= B[i]; //초 단위로 변경
			}
		}
		request.clear();
		for (int i = 0; i <= SIMULATION_TERM; i++)
		...
		ssd_use = 0;
		dram_use = 0;
		for (int i = 0; i <= SIMULATION_TERM + 3600; i++)
		{
			only_disk_use[i] = 0;
			disk_use[i] = 0;
			disk_req[i] = 0;
