...
string GetInputFileName(int ssdSize){
    return "SPC_poisson___(SSD=" + to_string(ssdSize) + ").txt";
}

string GetOutputFileName(int ssdSize, int ssdBandwidthIdx){
	return "SPC_Disk_bandwidth_saved___(SSD=" + to_string(ssdSize) + "_ssdBandwidth=" + to_string(memSize[maxSSDidx[ssdBandwidthIdx]]) + "KB).txt";
}
