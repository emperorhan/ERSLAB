...
string GetOutputFileName(int period){
    return "LFUDA_poisson_" + to_string(period) + ".txt";
}

int main(int argc, char* argv[]) {
	int nChange = atoi(argv[1]);
	freopen(GetOutputFileName(nChange).c_str(), "w+", stdout);

	for (int i = 1; i <= NUMBER_OF_VIDEO; i++) curP[i - 1] = getP(nChange - 1, i);
