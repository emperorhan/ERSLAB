...
string GetInputFileName(int period){
    return "LFUDA_poisson_" + to_string(period) + ".txt";
}
...
string GetOutputFileName(int ssdSize, int ssdBandwidthIdx, int period){
	...
	else if(ssdSize == half*2){
		if(ssdBandwidthIdx == 0) 
			ret += "1TB_2GB/";
		else 
			ret += "1TB_4GB/";
	}
	else if(ssdSize == half*3){
		if(ssdBandwidthIdx == 0) 
			ret += "1.5TB_2GB/";
		else 
			ret += "1.5TB_4GB/";
	}
	else if(ssdSize == half*4){
		if(ssdBandwidthIdx == 0) 
			ret += "2TB_2GB/";
		else 
			ret += "2TB_4GB/";
	}
	return ret + "LFUDA_Disk_bandwidth_saved___(N_CHANGE=" + to_string(period) + "_SSD=" + to_string(ssdSize) + "_ssdBandwidth=" + to_string(memSize[maxSSDidx[ssdBandwidthIdx]]) + "KB).txt";
}
